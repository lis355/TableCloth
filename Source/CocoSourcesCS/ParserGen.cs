...
public class ParserGen {
			...
			switch (p.typ) {
				...
				case Node.iter: {
					...
					if (p2.typ == Node.wt) {
						...
						gen.Write("WeakSeparator({0},{1},{2}) ", p2.sym.name, NewCondSet(s1), NewCondSet(s2));
    ...
    void GenProductions( bool generateProductionEvents = false )
    {
		foreach (Symbol sym in tab.nonterminals)
        {
			...
			gen.WriteLine(") {");
            if ( generateProductionEvents ) gen.Write( "\t\tProductionBegin( ENonTerminal." + sym.name + " );" + CR + LF );
			CopySourcePart(sym.semPos, 2);
			GenCode(sym.graph, 2, new BitArray(tab.terminals.Count));
            if ( generateProductionEvents ) gen.Write( "\t\tProductionEnd( ENonTerminal." + sym.name + " );" + CR + LF );
	...
	void InitSets() {
		for (int i = 0; i < symSet.Count; i++) {
			...
			int j = 0;
			foreach (Symbol sym in tab.terminals) {
				if (s[sym.n]) gen.Write("T,"); else gen.Write("F,");
				++j;
				if (j%4 == 0) gen.Write(" ");
			}
			if (i == symSet.Count-1) gen.WriteLine("F}"); else gen.WriteLine("F},");
		}
	}
